 ...
 * backend s'affichent tels quels (FE-04).
 */
...
export function OpRisk() {
  ...
  return <div>
    ...
    <h3>Octopulse OpRisk — incidents opérationnels (dossiers tracés, taxonomie Bâle du tenant)</h3>
    <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
      <button style={petit} onClick={charger} disabled={isDemoMode()}>Charger</button>
      <input placeholder="titre de l'incident" value={titre} onChange={(e) => setTitre(e.target.value)} style={petit}/>
      <input placeholder="catégorie Bâle" value={categorie} onChange={(e) => setCategorie(e.target.value)} style={petit}/>
    ...
    {cellules && <div>
      <h4 style={{ fontSize: 13 }}>Heatmap (CALCULÉE serveur — fréquence × sévérité, rejouable à date ; aucune cellule ne se saisit)</h4>
      <table style={{ borderCollapse: "collapse" }}><tbody>
        <tr>{["catégorie", "fréquence", "sévérité max", "score"].map((h) => <td key={h} style={{ ...td, color: tokens.color.muted }}>{h}</td>)}</tr>
    ...
    {incidents && <div>
      <h4 style={{ fontSize: 13 }}>Incidents ({incidents.length})</h4>
      <ul style={{ paddingLeft: 16 }}>{incidents.map((i) => <li key={i.id} style={petit}>
        <strong>{i.titre}</strong> — {i.categorie} · sévérité {i.severite} · {i.statut}
        ...
        }}>{i.statut === "DECLARE" ? "Analyser" : "Clore (motivé)"}</button>}
      </li>)}</ul>
    </div>}
    {actions && actions.length > 0 && <div>
      <h4 style={{ fontSize: 13 }}>Plan d&apos;action (le retard est un fait calculé — notifié, jamais bloquant)</h4>
      <ul style={{ paddingLeft: 16 }}>{actions.map((a) => <li key={a.id} style={petit}>
        {a.titre} — {a.statut}{a.enRetard && <strong style={{ color: tokens.color.danger }}> · EN RETARD</strong>}
      </li>)}</ul>
    </div>}
  </div>;
}
