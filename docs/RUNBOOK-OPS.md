...
> **Correctif d'infrastructure (fond, ratifié par la nécessité)** : `PrismaService` n'avait PAS de `onModuleDestroy` → chaque `app.close()` laissait ses connexions ouvertes ; en e2e (5 suites × ≈18 clients — un par module câblant sa PrismaService), elles s'accumulaient jusqu'à saturer Postgres (« too many clients »). Ajout de `onModuleDestroy(){ $disconnect() }` (libère au shutdown) + `connection_limit=1` sur la `DATABASE_URL` de test/CI (plafond par client). **Dette signalée** : un `PrismaModule` @Global (client unique) reste le correctif d'architecture — lot dédié.
