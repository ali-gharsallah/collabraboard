...
| 4. e2e Postgres réel | `npm run test:e2e:setup && npm run test:e2e` | **14/14** (kyc-rules 6 + FAT recette Vague 1 = 8, cf. docs/tests/) — exige le patch `kyc.controller` (guard `validate` retiré, sinon 403≠409) |
...
(plus aucun advisory) : lint + typecheck, `prisma:post`, `test:rules` (425), e2e (14/14),
