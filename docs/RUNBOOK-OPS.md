...
| 4. e2e Postgres réel | `npm run test:e2e:setup && npm run test:e2e` | **20/20** (kyc-rules 6 + FAT Vague 1 = 10 + FAT Vague 2 = 4, cf. docs/tests/) — exige le patch `kyc.controller` (guard `validate` retiré, sinon 403≠409) |
...
(plus aucun advisory) : lint + typecheck, `prisma:post`, `test:rules` (425), e2e (20/20),
