...
| 4. e2e Postgres réel | `npm run test:e2e:setup && npm run test:e2e` | **37/37** (kyc-rules 6 + FAT V1 10 + V2 4 + V3 7 + V4 6 + V5 4, cf. docs/tests/) — exige le patch `kyc.controller` (guard `validate` retiré, sinon 403≠409) |
...
(plus aucun advisory) : lint + typecheck, `prisma:post`, `test:rules` (425), e2e (37/37),
