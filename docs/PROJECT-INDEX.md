...
| `docs/tests/COUVERTURE-REGLES.md` | Matrice traçabilité exigences → FAT + tests (V1 12/12 · V2 6/6) |
| `docs/tests/PREUVES/` | Sorties brutes horodatées (`fat-vague1-run.txt`, `fat-vague2-run.txt`, `e2e-complet.txt`) |
...
- **e2e (Postgres réel)** : 20/20 (kyc-rules 6 + FAT Vague 1 10 + FAT Vague 2 4).
- **FAT recette** : Vague 1 10/10 + Vague 2 4/4 = **14/14 PASS (100 %)**. Bandeau démo front : 9/9.
- **Écrans réels** : 8 (Vague 1 : Clients, KYC, Règles AML, File d'alertes, Rejeu KYC à date, Finance Islamique · Vague 2 : Dossiers de risque, Pièces GED).
- **Rejeu à date** : paramètres (R127) **ET** dossier KYC (`/kyc/:code/a-date`) — **OUI**.
- **Périmètre règles** : R1 → R221 · **77 routes backend** (+2 portes notes dossier).
