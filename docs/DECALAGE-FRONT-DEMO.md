...
- `command` (Command Center) · `prospection` / `prospect_*` (pré-prospection) · `crossborder` · `invest` (investigation financière) · `swiftlab` (SWIFT/SEPA) · `legal` (contrats) · `opprisk` (Octopulse) · `regwatch` (veille régl.) · `olivia` (AI Core) · `bi` (reporting sur mesure) · `wfaudit` / `auditit` (audit) · `paramnav` / `iamguide` / `ssoparam` (IAM/SSO) · `admin` / `editorconsole` · ✅ **`home` FAIT (2026-07-27)** — écran Accueil (projection par rôle, 8 tuiles réelles, HO-01..08 partiels ; T7/T8 bloquées par les données, cf. ECARTS)
