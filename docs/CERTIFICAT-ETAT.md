...
| 32 | **Workflow Instances** | **`GET /v1/workflow-instances`, `/:id`, `/:id/events`** (projection du workflow gouverné KYC : steps + visas R15 + timeline) | **visible** | ✅ réel (nouveau, V12) |
...
| e2e Postgres réel (… + V9 2 + V10 2 + V12 3) | **50 / 50** | `pnpm --filter api run test:e2e` |
...
- Backend : **~123 routes** (+ Workflow Instances V12 : liste/détail/events), **36 modules** en Postgres réel (0 mock). Frontend : **33 écrans** (… V10 : Ports, NBA · **V12 : Workflow Instances réel** — projection KYC · Tâches reste **FE-05 seed**). Écarts front + décisions A1 : `docs/ECARTS-FRONT.md` ; migrations : `docs/MIGRATION-FRONT.md`.
