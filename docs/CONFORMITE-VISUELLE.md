...
|---|---------|-------|---------|
| G1 | Nav | La maquette groupe la nav en sections latérales (Clients & Relations, Front & Croissance, Compliance & Risque, Wealth & Marchés, Data & Intelligence, Configuration) ; le shell React était un tab-switcher plat de démonstration. | **corrigé (2026-07-29, prototype en attente de ratification visuelle Ali)** — `router.tsx` : **menu latéral gauche groupé** repris de la maquette (`NAV`), 10 groupes repliables + 3 raccourcis, item actif surligné, i18n des libellés de groupe ajoutée (rapport nav 0 écart 3 langues). Le portail de login R296 reste un chantier séparé. |
| G5 | Tokens/Structure (shell) | Le contenu des écrans « nus » (tables, formulaires) n'avait aucun style global — rendu navigateur par défaut. | **corrigé (2026-07-29, prototype)** — `theme.css` (importé par `main.tsx`) : palette olive + typographie + tableaux + champs de la maquette, appliqués au markup SÉMANTIQUE → relève les ~72 écrans SANS les réécrire ; les styles inline existants restent prioritaires (aucune régression). Ressemblance par-écran plus poussée = chantier continu. |
