...
|------|-------|--------|--------------------------------|
| **0** (R335/RB) | Baseline, flags, rollback | **DÉJÀ (part.)** + à finir | Flags RB-01..05 verts, docs, tag posés (commit `f648937`). Backups/CI/336 e2e/R334 = existants. **Reste** : snapshot surface API + golden parcours. |
| **A** (R336/LK) | Locking optimiste | **À ADAPTER** | **Pas sur l'event store** (append-only, sans version d'agrégat, CRUD-primaire) mais sur les **tables d'état mutables** : colonne `version` + `UPDATE … WHERE id AND version` → `ConcurrencyConflictError` → 409. C'est exactement la voie que la spec réserve à OctoPulse. Cibles prioritaires : `kyc_files`, `kyc_locks`, `coc_files`, `tx_verdicts`, `risk_cases`. |
...
| **D** (existant) | Multi-tenancy RLS | **DÉJÀ (SQL) mais À ADAPTER (runtime)** | Policies `FORCE RLS` + `olive_app` + recette 4b = **prouvés au niveau SQL**. **MAIS non enforced au runtime** (app en superuser owner, GUC jamais posé). Adaptation = **fermer le piège** : (1) app connectée en `olive_app` (non-super, non-owner) ; (2) `forTenant()` réel = `SET LOCAL app.tenant_id` par transaction ; (3) tests `test_rls_not_bypassed_by_app_role`, `test_connection_pool_no_leak`, isolation croisée par table. Piloté par `FF_RLS_ENFORCED`. |
