...
| **B** (R337/IDM) | Idempotence commandes | **FAIT (mécanisme + preuve)** | Table `processed_commands` (migration expand-only) + helper `executerIdempotent()` sous `FF_IDEMPOTENCY` : clé neuve → exécute + consomme DANS LA MÊME transaction ; rejeu même payload → réponse snapshotée, aucun effet ; payload différent → `IdempotencyKeyReuseError` → **filtre 422** ; échec → clé NON consommée (retry). Prouvé **IDM-01..03 + filtre** (harnais) et **IDM-04/05** (e2e : rejeu réel = 1 seul effet ; atomicité). Adoption progressive aux handlers de mutation (en-tête `Idempotency-Key`), consignée. |
| **C** (R338/PJ) | Projections reconstructibles | **FAIT (reconstructibilité prouvée + doc ; synchrone = N/A)** | Projections **event-derived** (golden record R104, case proposals) via outbox **ASYNC** — l'exigence spec « synchrone, sans bus » reste **N/A** (choix assumé). Livré : (a) `docs/outbox-projections.md` (modèle outbox R285/R286 + reconstructibilité) ; (b) **reconstructibilité** prouvée sur golden-record — **PJ-01** rebuild (rejeu journal) ≡ drain incrémental (fonction déterministe), **PJ-02** idempotence EN ÉTAT (N rebuilds convergent + GR-03 no-diff), **PJ-03** le CRUD n'est PAS reconstructible (événement = référence R285, pas payload → rejeu orphelin ne recrée rien). Aucun rebuild du CRUD (non dérivé d'événements). |
...
| **E** (R339/EV) | Versioning/upcasting | **FAIT (additif, sans flag)** | `event_version` sur `domain_events` (migration expand-only `…03`, `DEFAULT 1` = backfill de l'historique sans réécriture) + registre d'upcasters **TS** `payload→payload` immuable, chaînable par `type@fromVersion`, désérialisation **centralisée** `deserialiser()` — upcast à la **LECTURE seule** (append-only R48). Prouvé : **EV-01** (e2e : tout événement porte `event_version ≥ 1`), **EV-02/03/03b** (harnais : fixtures legacy lisibles · chaîne v1→v2→v3 additive · registre immuable), **EV-04** (e2e : événement legacy injecté → toujours lisible). `docs/event-versioning.md`. Écart consigné : upcasters TS (pas Pydantic), pas de reconstruction d'état (CRUD). |
